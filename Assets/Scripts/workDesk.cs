...
public class workDesk : MonoBehaviour
{
    ...
    public void Work()
    {
        customer_anim.SetBool("eat", true);  // Changed animation to eating

        InvokeRepeating("ConsumeFoods", 2f, 1f);  // Changed to ConsumeFoods
    ...
    void ConsumeFoods()
    {
        if (transform.childCount > 0)
        {
            Destroy(transform.GetChild(transform.childCount - 1).gameObject, 1f);  // Consume foods (destroy)
        }
        else
        {
            customer_anim.SetBool("eat", false);  // Stop eating
